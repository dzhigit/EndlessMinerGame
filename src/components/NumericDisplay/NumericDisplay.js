...
function formatNumber(number = 0, shortForm = true) {
    ...
    if (shortForm) {
        ...
        if (Math.abs(number) < 1000) {
            ...
            return Math.min(999, Math.max(-999, Math.round(Math.abs(number) * Math.sign(number)))).toString(); //return number rounded to nearest integer
        }
        else {
            ...
            return (Math.min(999, Math.max(-999, Number(Math.abs(Number(roundedNumber)) *
                Math.sign(Number(roundedNumber))))).toPrecision(3) + prefixes[exponent3 / 3]); //return coefficient of engineering notation with numeric prefix
        }
    }
    else {
...
const NumericDisplay = ({ value, shortForm }) => (_jsx("span", { className: styles.NumericDisplay, "data-testid": "NumericDisplay", children: formatNumber(value, shortForm) }));
...
//# sourceMappingURL=NumericDisplay.js.map